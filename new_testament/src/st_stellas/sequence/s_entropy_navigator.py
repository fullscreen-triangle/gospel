...
def _get_test_sequences_navigation():
    """Get predefined test sequences for S-entropy navigation"""
    return [
        'ATGCATGCATGCATGCATGCATGC',  # Low entropy (repetitive pattern)
        'AGCTTAGCTAGCTAGCTAGCTAGC',  # Medium entropy (alternating)
        ''.join(np.random.choice(['A', 'T', 'G', 'C'], 50)),  # High entropy (random)
        'AAAAAAAAAAAAAAAAAAAAAAAAA',  # Minimal entropy (homopolymer)
        'ATCGATCGATCGATCGATCGATCG',  # Low entropy (dinucleotide repeat)
        'GCGCGCGCGCGCGCGCGCGCGCGC',  # Low entropy (CG repeat)
        'ATGGCGTTTCACTTCTGAGTTCGG',  # Biological-like sequence
        'TATAAAAGGGCCCCTTTTAAATAT',  # Structured pattern
        'CGTACGTACGTACGTACGTACGTA',  # Trinucleotide repeat
        ''.join(np.random.choice(['A', 'T'], 30)),  # Binary entropy
        ''.join(np.random.choice(['G', 'C'], 30)),  # Binary entropy (GC)
        'AGTCAGTCAGTCAGTCAGTCAGTC',  # Quaternary repeat
    ]

def _load_sequences_from_file_navigation(filepath: str):
    """Load sequences from FASTA or text file for navigation analysis"""
    sequences = []
...
def _generate_navigation_test_sequences(n_sequences: int):
    """Generate test sequences for S-entropy navigation"""

    sequences = []
    bases = ['A', 'T', 'G', 'C']

    # Generate sequences with different entropy characteristics
    for i in range(n_sequences):
        seq_type = i % 6  # Cycle through 6 different entropy types

        if seq_type == 0:
            # Low entropy - homopolymer runs
            base = np.random.choice(bases)
            run_length = np.random.randint(20, 60)
            sequence = base * run_length
        elif seq_type == 1:
            # Low entropy - dinucleotide repeats
            dinuc = ''.join(np.random.choice(bases, size=2))
            repeats = np.random.randint(15, 35)
            sequence = dinuc * repeats
        elif seq_type == 2:
            # Medium entropy - structured patterns
            motif = ''.join(np.random.choice(bases, size=np.random.randint(4, 8)))
            repeats = np.random.randint(8, 20)
            noise_bases = int(0.2 * len(motif) * repeats)  # 20% noise
            base_seq = motif * repeats
            # Add random mutations
            seq_list = list(base_seq)
            for _ in range(noise_bases):
                if seq_list:
            ...
            # Variable entropy - mixed regions
            low_entropy_part = 'ATGC' * np.random.randint(5, 15)
            high_entropy_part = ''.join(np.random.choice(bases, size=np.random.randint(20, 50)))
            sequence = low_entropy_part + high_entropy_part
        else:
            # Biological-like patterns with variable entropy
            start_codon = 'ATG'
            coding_region = ''.join([''.join(np.random.choice(bases, size=3)) for _ in range(np.random.randint(10, 30))])
            polyA = 'A' * np.random.randint(5, 20)
            sequence = start_codon + coding_region + polyA
