...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_01_separation.pdf}
  \caption{Homolog--non-homolog separation as a function of substitution
  rate. Panel (a): ROC-AUC of within-family vs.\ between-family pairwise
  cosine similarities for DNA (blue) and protein (red). Panel (b): Cohen's
  $d$ effect size. The method is saturating (AUC $=1$) up to $\mu \approx
  0.10$ and retains $\mathrm{AUC} \ge 0.97$ through $\mu = 0.20$, beyond
  which protein performance degrades faster than DNA.}
  \label{fig:sep}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_02_recall.pdf}
  \caption{Recall of true family members among the top-$K$ neighbours
  returned by the shader kernel, averaged over queries. Panel (a): DNA;
  panel (b): protein. Grey crosses show recall@$5$ for $k$-mer Jaccard
  (exhaustive baseline).}
  \label{fig:recall}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_03_scaling.pdf}
  \caption{Per-query wall-clock time as a function of database size.
  Panel (a): absolute time in milliseconds. Panel (b): speedup factor of
  the shader kernel over the two baselines.}
  \label{fig:scaling}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.7\linewidth]{figures/fig_04_prefix.pdf}
  \caption{Hierarchical prefix addressing. Blue: recall of true family
  members among all sequences inside the query's prefix cell, with
  $1$-bit radius expansion; red (log axis): fraction of the database probed
  under the same prefix. Deeper prefixes filter aggressively but lose
  recall unless paired with a larger radius.}
  \label{fig:prefix}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\linewidth]{figures/fig_05_pareto.pdf}
  \caption{Speed--recall Pareto: total per-query wall time versus recall@10
  for each pipeline configuration. The shader-only first stage (leftmost
  blue point without the SW rerank contribution) is sub-millisecond;
  adding SW rerank on $20$ candidates gives recall@10 = 0.95 at
  $\approx 667$~ms, dominated by Python-level SW.}
  \label{fig:pareto}
\end{figure}
