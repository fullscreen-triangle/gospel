...
\newtheorem{drule}[theorem]{Rule}

\theoremstyle{remark}
\newtheorem{remark}[theorem]{Remark}
\newtheorem{convention}[theorem]{Convention}

\crefname{drule}{Rule}{Rules}
\Crefname{drule}{Rule}{Rules}
...
\begin{remark}[Measured decay]
\label{rem:decaymeasured}
Running the three sequences for $40$ steps from a common start makes the
separation concrete: the $1/k$ expansion drives $\flo_k(v)$ to
$0.025$, the $1/k^2$ expansion to $6.25\times10^{-4}$, while a
floor-stabilising expansion holds it at $1.0$ exactly
(\Cref{sec:validation}, E06). The gap is three orders of magnitude after
forty steps and widens without limit; the hypothesis of
\Cref{def:stabilising} is doing real work, not tidying an edge case.
\end{remark}
...
\begin{remark}[How often, and why the rate is a minority]
\label{rem:receiverrate}
It would be a misreading of \Cref{thm:receiver} to expect that changing
the receiver usually changes the value. It does not. Perturbing a single
background edge --- one not incident to $u$ --- moves $\sep(u)$ only when
that edge crosses $u$'s minimising cut, and most edges do not. Sweeping
a perturbation across every non-incident edge of a population of random
contact structures, $206$ of $1181$ edges ($17.4\%$) moved $\sep(u)$ at
all; where an edge did move it, the smallest displacement recorded was
$0.022$. Independently, over $150$ receiver pairs the value disagreed in
$26$ cases ($17.3\%$), and where it disagreed the discrepancy was never
small --- the smallest was $0.056$, five orders of magnitude above
numerical tolerance (\Cref{sec:validation}, E10).

Both halves of this matter. That the rate is a minority is why
receiver-relativity is easy to overlook in practice and easy to
misattribute to noise when it appears. That the discrepancies are never
marginal is what makes \Cref{cor:notnoise} substantive. The observed
displacements are broadly spread rather than concentrated, but they are
bounded away from zero: either the receivers agree exactly, or they
disagree by an amount no measurement programme will close. There is no
population of near-misses in between, which is precisely what a noise
...
\label{ass:effective}
There is $\eta>0$ such that whenever $U(\dem)$ is defined, either
\[
  U(\dem)\;\le\;\dem-\eta
  \qquad\text{or}\qquad
  U(\dem)\;\le\;\theta .
...
\begin{remark}[Why the second disjunct is not redundant]
\label{rem:lastdisjunct}
The bare condition $U(\dem)\le\dem-\eta$ is what the termination argument
needs and is all \Cref{thm:mono} uses, but it cannot be imposed as a
runtime check: on the final step, when $\dem<\eta$, an update that lands
on quiescence lands on a value that is not below the negative quantity
$\dem-\eta$. A checker enforcing the first disjunct unconditionally
reports a violation on the last step of every terminating run --- which
is what ours did before we separated the two cases. The disjunctive form
is the assumption that is simultaneously strong enough for
\Cref{thm:dichotomy} and enforceable, and it is the form the language
checks.
\end{remark}
...
$U(\dem_k)$ is defined then, since the loop has not terminated we have
$\dem_k>\theta$, so the second disjunct of \Cref{ass:effective} would
end the run and the bound is asserted only where the first applies:
...
\emph{Response.} Let the receiver $N$ be $P$ itself, padded with a fixed
block of $n_f$ filler items mutually joined at weight $w_f$ and each
joined to the medium at $w_f$, with $w_f>\flo$. Let the response
perturbation $\delta_c$ raise \emph{both} pendant edges
$\{u,\med\}$ and $\{v,\med\}$ by $c$. By \Cref{prop:skeleton} the
skeleton is unchanged, so $N$ and $N_c$ are comparable edge for edge
under the identity correspondence.

Both pendants must be raised. If only one is raised, the minimum
$(u,v)$-cut is realised by a separating set that places the medium on
the raised side and severs only the untouched pendant edge; its weight
is $\flo$ for every $c$, and $\Delta$ does not move at all. This is not
a defect of the bound but of the witness, and it is the reason the
one-sided construction --- the natural first attempt --- registers no
response whatever.

With both raised, every $(u,v)$-separating set severs at least one edge
of weight $\flo+c$, so $\sep_{N_c}(u\mid v)=\flo+c$, while
$\Om_c=\Om_0+2c$ and the floor is
$\flo_c=\min\{\flo+c,\,w_f\}$. Hence
\begin{equation}
\label{eq:ffcurve}
  \Delta_c(u,v)\;=\;\frac{\flo+c}{\Om_0+2c}\;-\;\frac{\min\{\flo+c,\;w_f\}}{\Om_0+2c}
  \;=\;\frac{\max\{0,\;\flo+c-w_f\}}{\Om_0+2c},
\end{equation}
which is identically $0$ for $c\le w_f-\flo$ and thereafter strictly
increasing to the limit $\tfrac12$. The flat initial segment is
\Cref{thm:floor} operating correctly rather than an absent response:
while the pendant edges are still the lightest in the structure they
\emph{are} the floor, so $\flo_c$ rises in lockstep with the cut and the
difference cannot open. The response becomes visible exactly once the
pendants exceed the surrounding weight $w_f$.

The divergence is therefore in the normalised gap, which is bounded by
$\tfrac12$ on any single pendant pair; to obtain an unbounded
$\dem^{\mathrm{response}}$ it suffices to take a residue of $m$
independent pendant pairs, each contributing a term of
\eqref{eq:ffcurve}, so that
$\dem^{\mathrm{response}}\ge m\,\Delta_c\to\infty$ with $m$. What the
theorem requires, and what \eqref{eq:ffcurve} supplies, is that the
response separates without bound while the central demand is pinned at
zero.
\end{proof}

\begin{remark}[Measured]
\label{rem:ffmeasured}
On the instance $\flo=0.2$, $n_f=3$, $w_f=1$, sweeping $c$ over
$[1,5]$ gives $\Delta_c$ rising from $0.0238$ to $0.2561$ while
$\dem^{\mathrm{central}}$ is $0$ to machine precision throughout
(\Cref{sec:validation}, E16). Below $c=w_f-\flo$ the response is exactly
$0$, as \eqref{eq:ffcurve} predicts.
\end{remark}
...
\label{cor:converse}
There is a family indexed by a bridge weight $d\ge0$ with
$\dem^{\mathrm{central}}$ strictly increasing in $d$ over an initial
interval, and $\dem^{\mathrm{response}}=0$ for all $d$.
\end{corollary}

\begin{proof}
Take the two-cluster structure of \Cref{thm:region}: two $r$-cliques at
intra-weight $W$, joined by the single bridge $\{a_0,b_0\}$, every item
joined to the medium at $\flo$. Put $u=a_1$, $v=b_1$ and index the
family by the bridge weight $d$, so that the bridge carries $\flo+d$.

The index must be the bridge, not $W$. Raising $W$ leaves the minimum
$(a_1,b_1)$-cut fixed --- each of $a_1,b_1$ detaches through the medium
at $\flo$ regardless of how heavy its own clique becomes --- while
$\Om=r(r-1)W+\flo+2r\flo$ grows linearly in $W$. So $a(u,v)$ and hence
$\Delta$ \emph{fall} as $W$ increases. The bridge is the edge that the
$(a_1,b_1)$-cut actually crosses, and it is therefore the parameter the
central demand responds to.

With $d$ as index, any $(a_1,b_1)$-separating set must either sever
$a_1$ from the medium and from its clique, or cross the bridge; in the
regime where the latter is minimal the cut weight is affine in $d$ with
positive coefficient while $\Om$ grows at the same absolute rate on a
single edge. Since the cut is a minimum of finitely many affine
functions of $d$, $a(u,v)$ is piecewise affine and strictly increasing
until the minimiser switches to a $d$-independent set, after which it
is non-increasing. Restricting to the initial interval gives strict
increase, as claimed.
...
\begin{remark}[Measured]
\label{rem:convmeasured}
At $r=4$, $W=1$, $\flo=0.2$, sweeping the bridge $d$ over $[0,2]$ raises
$\Delta(a_1,b_1)$ monotonically from $0.0580$ to $0.1772$, with
$\dem^{\mathrm{response}}=0$ throughout (\Cref{sec:validation}, E17).
Sweeping $W$ instead over $[0.5,6]$ sends $\Delta$ from $0.1026$
\emph{down} to $0.0108$, with the cut constant at $1.0$ and $\Om$
growing from $7.8$ to $73.8$ --- which is why the corollary is indexed
as it is. Continuing the bridge sweep past $d\approx2$ turns the curve
over as the minimiser switches, so the strict increase is genuinely
local, not global.
\end{remark}
...
\begin{remark}[Measured]
\label{rem:orthmeasured}
Both axes are occupied strictly, not merely approached. Over the
both-pendant family of \Cref{rem:ffmeasured} at $c\in[1.5,5]$, every
member has $\dem^{\mathrm{central}}=0$ to machine precision and
$\dem^{\mathrm{response}}>0$ strictly; over the bridge family of
\Cref{rem:convmeasured} at $d\in[0.25,2]$, every member has
$\dem^{\mathrm{response}}=0$ and $\dem^{\mathrm{central}}>0$ strictly
(\Cref{sec:validation}, E18). The stronger per-axis statement matters:
a family that merely avoided the origin would be consistent with the two
demands being correlated, which is what \Cref{def:fourq} is built to
exclude.
\end{remark}
...
\begin{drule}[Sequences are opaque]
\label{rule:opaque}
...
\begin{drule}[Every value carries residue]
\label{rule:residue}
...
\begin{drule}[No zero-residue constructor]
\label{rule:nozero}
...
\begin{drule}[Verdicts are four-ary]
\label{rule:arity}
...
\begin{drule}[Frames are lexical]
\label{rule:frames}
...
\begin{drule}[No default thresholds]
...
\begin{drule}[Bounded iteration only]
\label{rule:bounded}
...
\begin{drule}[Relaxation must be floored]
\label{rule:floored}
...
\begin{drule}[Three result types]
\label{rule:three}
...
\begin{drule}[Programs emit reports]
\label{rule:report}
...
Every claim in this paper that can be checked by computation has been.
This section states what was checked, how, and --- because this is the
part that carries information --- what the checking changed.

\subsection{What is checkable, and what is not}

The claims fall into three kinds, and conflating them would make the
exercise worthless.

\textbf{Mathematical} (\Cref{sec:contact}--\Cref{sec:separation}). These
are statements about contact structures and are checkable numerically on
explicit instances. Minimum cuts are computed by exhaustive enumeration
over separating subsets rather than by max-flow: the cut is
\emph{defined} as a minimum over separating sets (\Cref{def:contact}),
and an approximate solver agreeing with an approximate expectation could
conceal a real disagreement. This is exponential, and affordable only on
the small structures the validation uses --- which is the point of using
them.

\textbf{Language} (\Cref{sec:lang}--\Cref{sec:meta}). The refusal
theorems say certain programs \emph{do not typecheck}. A checker that
rejected everything would satisfy every refusal theorem vacuously, so
each negative program is paired with a positive one, and the four
programs of \Cref{sec:programs} are required to typecheck
\emph{verbatim as printed}. A refusal counts only if it carries the
expected error class: rejecting a cross-frame reference as a parse error
would not be \Cref{thm:scope}.

\textbf{Measurement} (\Cref{sec:open}). \Cref{ass:respind} is reported,
not asserted. Recording it as an assertion would either bake in a
statement we believe to be false or hide the paper's principal finding.
...
\midrule
Claim categories & $32$ \\
\quad of which assertions & $31$ \\
\quad of which measurements & $1$ \\
Individual checks & $5622$ \\
Checks passed & $5622$ \\
Assertion categories failed & $0$ \\
Random seed & $20260816$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent Of the language checks, the four programs of
\Cref{sec:programs} typecheck as printed, and $16$ negative programs are
each rejected with the specific error class the corresponding theorem
names --- arity, scope, residue, parameter and termination violations
among them.

\subsection{What the checking changed}

A validation that confirms everything has usually tested nothing. Three
proofs in this paper were wrong when first written, and the corrections
are recorded in place rather than quietly absorbed.

\begin{itemize}[leftmargin=1.4em]
\item \Cref{thm:falsefriend}: the original proof perturbed \emph{one}
      pendant edge. That leaves the minimum cut pinned by the other
      pendant --- the minimiser simply places the medium on the raised
      side --- and the response is identically zero for every $c$. The
      theorem is true; the witness was not. See \Cref{rem:ffmeasured}.
\item \Cref{cor:converse}: the original proof indexed the family by the
      intra-cluster weight $W$ and asserted that
      $\dem^{\mathrm{central}}$ increases in $W$. It decreases: the cut
      is constant while $\Om$ grows. The family must be indexed by the
      bridge, which is the edge the cut crosses. See
      \Cref{rem:convmeasured}.
\item \Cref{thm:region}: an earlier version claimed a light edge between
      non-pendant items places them at the floor. Exact enumeration gave
      $1.4$ where the argument predicted $0.2$. This is now recorded as
      \Cref{rem:badwitness}, and it is why \Cref{thm:falsefriend} insists
      on pendants.
\end{itemize}

\noindent A fourth correction was to a check rather than a proof. We had
demanded that receiver disagreement occur in at least $20\%$ of cases;
no theorem says that, and \Cref{rem:receiverrate} reports what is
actually true --- a minority rate, with discrepancies that are never
marginal when they occur. A threshold that encodes no claim is a
threshold that will eventually be tuned until it passes.
...
\subsection{The same failure on a second corpus}

The biochemical networks are few and are not independent of one another;
a reader is entitled to ask whether the result is a property of those
four pathways. It is not. Repeating the construction on $200$ random
contact structures, with no biochemical content whatever, gives $361$
comparisons and the same verdict (\Cref{sec:validation}, E32):

\begin{center}
\begin{tabular}{@{}lrr@{}}
\toprule
Quantity & Networks & Random \\
\midrule
Comparisons & $35$ & $361$ \\
Within $\theta=0.01$ & $28.6\%$ & $77.8\%$ \\
Mean discrepancy & $0.070$ & $0.00555$ \\
Median discrepancy & --- & $0.00451$ \\
Maximum discrepancy & $0.307$ & $0.0360$ \\
Assumption holds at $\theta$ & no & no \\
\bottomrule
\end{tabular}
\end{center}

The two corpora differ in severity and agree in kind. On random
structures the assumption holds far more often --- $77.8\%$ against
$28.6\%$ --- and the discrepancies are an order of magnitude smaller,
which is what one expects when the perturbed edges are not selected for
biological significance. But $77.8\%$ is not $100\%$, the maximum is
still more than triple $\theta$, and the failure rate does not fall with
sample size. \Cref{ass:respind} is not approximately true and awaiting a
sharper estimate; it is false, and more uniformly false the more
structured the network.

This also disposes of the most natural rescue. If the discrepancies were
sampling error, enlarging the corpus by an order of magnitude would
shrink them. It does not, and by \Cref{cor:notnoise} it cannot: each
value is exactly determined by its own structure, so there is nothing
for more sampling to average away.
